...
function ensureManagerActions(){if($('#saveExamSettings'))return;const anchor=$('#examChoice').closest('label'),saveButton=document.createElement('button'),deleteButton=document.createElement('button');saveButton.id='saveExamSettings';saveButton.type='button';saveButton.className='primary';saveButton.textContent='試験設定を保存';deleteButton.id='deleteExam';deleteButton.type='button';deleteButton.className='danger';deleteButton.textContent='この試験を削除';anchor.insertAdjacentElement('afterend',deleteButton);anchor.insertAdjacentElement('afterend',saveButton);saveButton.addEventListener('click',()=>saveExamSettings().catch(e=>show($('#message'),e.message,'error')));deleteButton.addEventListener('click',()=>deleteCurrentExam().catch(e=>show($('#message'),e.message,'error')));const copyButton=document.createElement('button');copyButton.id='copyExam';copyButton.type='button';copyButton.textContent='選択中の試験をコピー';$('#createExam').insertAdjacentElement('afterend',copyButton);copyButton.addEventListener('click',()=>copyCurrentExam().catch(e=>show($('#message'),e.message,'error')))}
async function saveExamSettings(){await registerExamName();const option=$('#examChoice').selectedOptions[0];if(option)option.textContent=exam.examName;show($('#message'),'試験設定を保存しました。')}
function destinationValues(){const examId=$('#newExamId').value.normalize('NFKC').trim().toUpperCase(),examName=$('#newExamName').value.trim();if(!/^[A-Z0-9_-]+$/.test(examId))throw Error('コピー先の試験IDは半角英数字・_・-で入力してください');if(!examName)throw Error('コピー先の試験名を入力してください');return{examId,examName}}
async function copyCurrentExam(){const{examId,examName}=destinationValues(),list=await loadExamList();if(list.some(x=>x.examId===examId))throw Error('同じ試験IDがすでに登録されています');const copiedExam={...exam,examId,examName,version:'1.0.0',updatedAt:new Date().toISOString(),isPublished:true,availableFrom:null,availableUntil:null},copiedQuestions=questions.map(q=>({...q,id:`${examId.toLowerCase()}-${q.section}-${String(q.number).padStart(3,'0')}`,examId,category:$('[data-category="'+q.id+'"]').value})),response=await fetch(`/api/exams/${encodeURIComponent(examId)}`,{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({exam:copiedExam,questions:copiedQuestions})});if(!response.ok)throw Error(`試験のコピーに失敗しました (${response.status})`);const option=document.createElement('option');option.value=examId;option.textContent=examName;$('#examChoice').append(option);await loadAdminExam(examId);show($('#message'),`${examName}をコピーしました。正答と平均値は未登録です。`)}
async function deleteCurrentExam(){if(!exam)return;if(!confirm(`「${exam.examName}」を削除します。試験設定・問題設定・登録済み正答・平均は元に戻せません。`))return;const response=await fetch(`/api/exams/${encodeURIComponent(exam.examId)}`,{method:'DELETE'});if(!response.ok)throw Error(`試験の削除に失敗しました (${response.status})`);const list=await loadExamList();$('#examChoice').innerHTML=list.map(x=>`<option value="${esc(x.examId)}">${esc(x.examName)}</option>`).join('');if(!list.length){location.reload();return}await loadAdminExam(list[0].examId);show($('#message'),'試験を削除しました。')}
async function loadAdminExam(id){({exam,questions}=await loadBase(id));render();$('#examName').value=exam.examName;fillManager();ensureManagerActions();await restore()}
async function createExam(){const examId=$('#newExamId').value.normalize('NFKC').trim().toUpperCase(),examName=$('#newExamName').value.trim(),type=$('#newExamType').value,am=Number($('#newAm').value),pm=Number($('#newPm').value);if(!/^[A-Z0-9_-]+$/.test(examId))throw Error('試験IDは半角英数字・_・-で入力してください');if(!examName)throw Error('試験名を入力してください');if(!Number.isInteger(am)||!Number.isInteger(pm)||am<0||pm<0||am+pm<1)throw Error('問題数が不正です');const created={schemaVersion:'1.0',examId,examName,examType:type,version:'1.0.0',formatVersion:'1',updatedAt:new Date().toISOString(),isPublished:true,availableFrom:null,availableUntil:null,sections:[{id:'am',name:'午前',questionCount:am},{id:'pm',name:'午後',questionCount:pm}],categories},createdQuestions=generatedQuestions(examId,type,am,pm),response=await fetch(`/api/exams/${encodeURIComponent(examId)}`,{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({exam:created,questions:createdQuestions})});if(!response.ok)throw Error(`試験作成に失敗しました (${response.status})`);const option=document.createElement('option');option.value=examId;option.textContent=examName;$('#examChoice').append(option);await loadAdminExam(examId);show($('#message'),`${examName}を作成しました。正答を設定してください。`)}
