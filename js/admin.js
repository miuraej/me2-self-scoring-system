...
function ensureDeleteButton(){if($('#deleteExam'))return;const button=document.createElement('button');button.id='deleteExam';button.type='button';button.className='danger';button.textContent='この試験を削除';$('#examChoice').closest('label').insertAdjacentElement('afterend',button);button.addEventListener('click',()=>deleteCurrentExam().catch(e=>show($('#message'),e.message,'error')))}
...
async function loadAdminExam(id){({exam,questions}=await loadBase(id));render();$('#examName').value=exam.examName;fillManager();ensureDeleteButton();await restore()}
