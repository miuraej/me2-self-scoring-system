...
export async function loadBase(){const [exam,questions]=await Promise.all([fetch('data/exam.json',{cache:'no-store'}).then(check).then(r=>r.json()),fetch('data/questions.json',{cache:'no-store'}).then(check).then(r=>r.json())]);const errors=validatePublicData(exam,questions);if(errors.length)throw Error(errors.join('、'));return{exam,questions}}
